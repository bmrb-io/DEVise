...
public:
  ...
  void add_entry(const char *schema);

  void add_topgrp(const char *schema, Group *gp);
  void top_level_groups(char *result, const char *schema);
  
  void get_items(char *result, const char *schema, const char *topgname,
                 const char *gname);
  int find_entry(const char *schema);
  int num_topgrp(const char *schema);

private:
  Group *find_grp(Group *gp, const char *gname);
};
