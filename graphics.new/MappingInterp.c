...
/* tcl usage: dayOfWeek num: returns day of week, given a number
that represents UNIX time. Sunday == 0 */
static int DayOfWeekCmd(ClientData clientData, Tcl_Interp *interp,
	int argc, char **argv){

	if (argc != 2){
		interp->result = "dayOfWeek: need 1 arg";
		return TCL_ERROR;
	}
	time_t clk = atoi(argv[1]);
	struct tm *tstr = localtime(&clk);
	sprintf(interp->result,"%d",tstr->tm_wday);
	return TCL_OK;
}

/* tcl usage: season num: return the season, given a number
that represents UNIX time. Spring == 0 */
static int SeasonCmd(ClientData clientData, Tcl_Interp *interp,
	int argc, char **argv){

	if (argc != 2) {
		interp->result = "season: need 1 arg";
		return TCL_ERROR;
	}
	time_t clk = atoi(argv[1]);
	struct tm *tstr = localtime(&clk);
	int month = tstr->tm_mon+1; /* month from 1-12 */
	...
	*/
	...
	if (month >= 3 && month <= 5)
		...
		interp->result = "2";
	else interp->result = "3";
	return TCL_OK;
}


/* tcl usage: random [num1] [num2]: returns a random number
between num1 and num2. default: 0.0 to 1.0.
option: random num2: num1 defaults to 0.0*/
static int RandomCmd(ClientData clientData, Tcl_Interp *interp,
	int argc, char **argv){
	...
	double rndNum = (double)(random() % 32767);
	sprintf(interp->result,"%f", (rndNum/32767.0)*(num2-num1)+num1);
	/*
	printf("random num1 %f num2 %f rndNum %f final %f\n", 
		num1, num2, rndNum, interp->result);
	*/
	return TCL_OK;
}
...
*/
	...
	if (_interp == NULL){
		...
		Tcl_LinkVar(_interp,"interpResult",(char *)&_interpResult, TCL_LINK_DOUBLE);
		...
		Tcl_CreateCommand(_interp,"random", RandomCmd,NULL,NULL);
		Tcl_CreateCommand(_interp,"dayOfWeek", DayOfWeekCmd,NULL,NULL);
		Tcl_CreateCommand(_interp,"season", SeasonCmd,NULL,NULL);
	}
	...
	if (info->isSorted) {
		...
		VisualFlag *vf = new VisualFlag;
		*vf = VISUAL_X;
		SetDimensionInfo(vf,1);
	}
	UpdateBoundingBox(0.1,0.1);
}
...
/*
void MappingInterp::UpdateBoundingBox(int pageNum, void **gdataArray, 
	int numRecs){
}
*/
...
void MappingInterp::DrawGDataArray(WindowRep *win, void **gdataArray, int num,
	...
	if (_offsets->shapeOffset < 0){
		...
		_shapes[shape]->DrawGDataArray(win,gdataArray,num,
			this, xPerPixel, yPerPixel, GetPixelWidth());
	}
	else {
		/* dynamic shape */
		i=0;
		while (i < num){
			ShapeID shape = 
				*((ShapeID *)(gdataArray[i]+_offsets->shapeOffset));
			int j;
			for (j=i+1; j < num; j++){
				ShapeID nextShape = 
			...
			printf("draw shape %d\n", shape);
			*/
			_shapes[shape]->DrawGDataArray(win,&gdataArray[i],
				j-i,this, xPerPixel, yPerPixel, GetPixelWidth());
			i = j;
		}
	}
}
...
*/
		if (_offsets->xOffset >= 0){
			if (_tclCmd->xCmd == NULL){
				...
				_interpResult = 0.0;
				code = Tcl_VarEval(_interp,
					"set interpResult ",_tclCmd->xCmd,NULL);
			}
			/*
			printf("eval x\n");
			*/
...
*/
		if (_offsets->yOffset >= 0){
			if (_tclCmd->yCmd == NULL){
				...
				_interpResult = 0.0;
				code = Tcl_VarEval(_interp,
					"set interpResult ",_tclCmd->yCmd,NULL);
			}
			/*
			printf("eval y\n");
			*/
		...
		if (_offsets->sizeOffset >= 0){
			if (_tclCmd->sizeCmd == NULL){
				...
				_interpResult = 0.0;
				code = Tcl_VarEval(_interp,"set interpResult ",_tclCmd->sizeCmd,
					NULL);
			}
			/*
			printf("eval size\n");
...
AttrList *MappingInterp::InitCmd(char *name){
	...
	if (_cmdFlag & MappingCmd_X){
		if (!ConvertSimpleCmd(_cmd->xCmd,_simpleCmd->xCmd, attrType,
			isSorted))
			goto complexCmd;
	...
	if (_cmdFlag & MappingCmd_Y){
		if (!ConvertSimpleCmd(_cmd->yCmd,_simpleCmd->yCmd, attrType,
			isSorted))
			goto complexCmd;
	...
	if (_cmdFlag & MappingCmd_Pattern){
		...
		if (_simpleCmd->patternCmd.cmdType == MappingSimpleCmdEntry::ConstCmd){
			...
			_offsets->patternOffset = offset = 
				WordBoundary(offset,sizeof(Pattern));
	...
	if (_cmdFlag & MappingCmd_Shape){
		...
		if (_simpleCmd->shapeCmd.cmdType == MappingSimpleCmdEntry::ConstCmd){
			...
			_offsets->shapeOffset = offset = 
				WordBoundary(offset,sizeof(ShapeID));
	...
	if (_cmdFlag & MappingCmd_Orientation){
		...
		if (_simpleCmd->orientationCmd.cmdType == 
			...
			_offsets->orientationOffset = offset = 
				WordBoundary(offset,sizeof(double));
	...
	for (j=0; j < MAX_GDATA_ATTRS; j++){
		...
		if (_cmdAttrFlag & (1<<j)){
			...
			if (_simpleCmd->shapeAttrCmd[j].cmdType == 
				MappingSimpleCmdEntry::ConstCmd){
				/* constant */
				SetDefaultShapeAttr(j,
					(Coord)_simpleCmd->shapeAttrCmd[j].cmd.num);
			} else {
				_offsets->shapeAttrOffset[j] = offset = 
					WordBoundary(offset,sizeof(double));
				attrList->InsertAttr(
					j+7,attrName,offset,sizeof(double),attrType,
						false,  NULL, false, isSorted);
				offset += sizeof(double);
			}
		}
...
complexCmd:
	...
	if (_cmdFlag & MappingCmd_Color){
		if (IsConstCmd(_cmd->colorCmd,constVal)){
			...
			_offsets->colorOffset = offset = 
				WordBoundary(offset,sizeof(Color));
			attrList->InsertAttr(2,"color",offset,sizeof(double),attrType,
				false,  NULL, false, isSorted);
			offset += sizeof(Color);
		}
...
Boolean MappingInterp::ConvertSimpleCmd(char *cmd, 
	MappingSimpleCmdEntry &entry, AttrType &type, Boolean &isSorted) {
	...
	while (*cmd != '\0'){
		if (*cmd == '$'){
			...
			if (ptr == cmd+1){
				...
				/* from cmd+1 to ptr is a variable name */
				char buf[80];
				int len = ptr-cmd;
				if (len > 80){
					fprintf(stderr,"can not hanld variable name > 80\n");
					Exit::DoExit(2);
				}
				char *bufPtr = buf;
				for (cmd++; cmd < ptr; cmd++, *bufPtr++){
					*bufPtr = *cmd;
				}
				*bufPtr = '\0';
				...
				if (info == NULL){
					...
					*/
					...
					else sprintf(buf,"$interpAttr_%d",info->attrNum);
					InsertString(buf);
				}
				cmd = ptr-1;
			}
...
*/
	...
	for (i=0;i < numRecs; i++){
		...
		if (_offsets->colorOffset >=0){
			cPtr = (Color *)(gPtr+_offsets->colorOffset);
		...
		if (_offsets->shapeOffset >=0){
			sPtr = (ShapeID *)(gPtr+_offsets->shapeOffset);
			*sPtr = (ShapeID) ConvertOne(tPtr, &_simpleCmd->shapeCmd, 1.0);
		}
		if (_offsets->patternOffset >=0){
			pPtr = (Pattern *)(gPtr+_offsets->patternOffset);
		...
		for (j=0; j < _maxGDataShapeAttrNum; j++){
			if (_offsets->shapeAttrOffset[j] >= 0){
				dPtr = (double *)(gPtr+_offsets->shapeAttrOffset[j]);
