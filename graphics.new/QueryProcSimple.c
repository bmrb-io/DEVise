...
Append query to end 
**************************************************************************/
void QueryProcSimple::AppendQuery(TDataMap *map, 
...
void QueryProcSimple::DoQueryDone(){
		...
		query->callback->QueryDone(query->bytes,FirstQuery()->userData);
		JournalReport();
		if (QueryListSize() <= 1 &&
...
************************************************************************/
Boolean QueryProcSimple::DoBinarySearch(BufMgr *mgr,
...
void QueryProcSimple::ProcessScatterQuery(){
	...
	query->callback->QueryDone(FirstQuery()->bytes,FirstQuery()->userData);
	JournalReport();
	if (QueryListSize() <= 1 && query->gdata != NULL){
		_state = MakeGDataNew;
	}
...
void QueryProcSimple::InitTDataQuery(TDataMap *map, 
	VisualFilter &filter, Boolean approximate){
