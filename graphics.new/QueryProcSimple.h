...
private:
	...
	Boolean _useExisting; /* TRUE if use existing pages in query processing */
	...
	***************************************************************/
	...
	/* Append query to end */
	void AppendQuery( TDataMap *map, 
		VisualFilter &filter, QueryCallback *callback, 
	...
	Boolean DoBinarySearch(BufMgr *mgr, TData *tdata, TDataMap *map,
		...
		Boolean bounded=false, RecId lowBound=0, RecId highBound=0);
	...
	Boolean DoYXBinarySearch(BufMgr *mgr,
	...
	/* record number being scanned, for either regular scan or binary search,
	RecId _tScanId;	/* for tdata */
