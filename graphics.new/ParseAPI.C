...
int ParseAPI(int argc, char **argv, ControlPanel *control)
{
  ...
  if (!strcmp(argv[0], "createInterp")) {
    ...
    if (!classInfo) {
      control->ReturnVal(API_ERROR, "Cannot create mapping");
      return API_ERROR;
    }
    ControlPanel::RegisterClass(classInfo, true);
    control->ReturnVal(API_OK, classInfo->ClassName());
    return API_OK;
  }
  ...
  if (!strcmp(argv[0], "create")) {
    ...
    control->SetIdle();
    if (!name)
      control->ReturnVal(API_OK, "");
    else
      control->ReturnVal(API_OK, name);
    return API_OK;
  }
  ...
  if (!strcmp(argv[0], "showkgraph")) {
    ...
    for(int i = 0; i < nview; i++) {
      ...
      if (!vlist[i]) {
	control->ReturnVal(API_ERROR, "Cannot find view");
	return API_ERROR;
      }
    }
    vkg->Init();
    ...
    if (vkg->AddViews(vlist, nview, argv[3]) == false) {
      control->ReturnVal(API_ERROR, "Could not add views to ViewKGraph");
      return API_ERROR;
    }
    
    if (vkg->Display(atoi(argv[2])) == false) {
      control->ReturnVal(API_ERROR, "Could not display the KGraph");
      return API_ERROR;
    }
    free(vlist);
    control->ReturnVal(API_OK, "done");
  ...
  if (!strcmp(argv[0], "createMappingClass")) {
    ...
    ClassInfo *classInfo = interp->CreateWithParams(argc-1,&argv[1]);
    if (!classInfo) {
      control->ReturnVal(API_ERROR, "Cannot create mapping class");
      return API_ERROR;
    }
    ControlPanel::RegisterClass(classInfo, true);
    control->ReturnVal(API_OK,classInfo->ClassName());
  ...
  if (argc == 1) {
    if (!strcmp(argv[0], "date")) {
      ...
      control->ReturnVal(API_OK,DateString(tm));
      return API_OK;
    }
    ...
    control->ReturnVal(API_ERROR, "wrong args");
    return API_ERROR;
  }
  ...
  if (argc == 2) {
	...
	result[len - 1] = '\0';
      control->ReturnVal(API_OK,result);
      return API_OK;
    }
    ...
    if (!strcmp(argv[0], "tdataFileName")) {
      ...
      if (!tdata) {
	control->ReturnVal(API_ERROR, "Cannot find tdata");
	return API_ERROR;
      }
      control->ReturnVal(API_OK,tdata->GetName());
      return API_OK;
    }
	...
	control->ReturnVal(API_OK,viewWin->GetName());
      return API_OK;
    }
    if (!strcmp(argv[0], "clearViewHistory")) {
      View *view = (View *)classDir->FindInstance(argv[1]);
      if (!view) {
	control->ReturnVal(API_ERROR, "Cannot find cursor");
	return API_ERROR;
      }
    ...
    if (!strcmp(argv[0], "getMappingTData")) {
      ...
      if (!map) {
	control->ReturnVal(API_ERROR, "Cannot find mapping");
	return API_ERROR;
      }
      TData *tdata = map->GetTData();
      control->ReturnVal(API_OK,classDir->FindInstanceName(tdata));
      return API_OK;
    }
    ...
    if (!strcmp(argv[0], "getWindowLayout")) {
      ...
      if (!layout) {
	control->ReturnVal(API_ERROR, "Cannot find window");
	return API_ERROR;
      }
      int v, h;
      Boolean stacked;
	...
	return API_ERROR;
      ...
      if (!attrList) {
	control->ReturnVal(API_ERROR, "Null attribute list");
	return API_ERROR;
      }
    ...
    if (!strcmp(argv[0], "getLinkFlag")) {
      ...
      if (!link) {
	control->ReturnVal(API_ERROR, "Cannot find link");
	return API_ERROR;
      }
    ...
    if (!strcmp(argv[0], "importFileType")) {
      ...
      if (!name) {
	strcpy(result , "");
	control->ReturnVal(API_ERROR, result);
	return API_ERROR;
      }
      control->ReturnVal(API_OK, name);
      return API_OK;
	...
	return API_ERROR;
      ...
      if (!view->Mapped()) {
	control->ReturnVal(API_ERROR, "View not in any window");
	return API_ERROR;
      }
	...
	return API_ERROR;
      ...
      control->ReturnVal(API_OK,vg->GetDisplayStats());
      return API_OK;
    }
  ...
  if (argc == 3) {
    ...
    if (!strcmp(argv[0], "open")) {
      ...
      if (!file) {
	control->ReturnVal(API_ERROR, "Cannot open file");
	return API_ERROR;
      }
	...
	return API_ERROR;
      ...
      control->ReturnVal(API_OK, "API_OK");
      return API_OK;
    }
    ...
    if (!strcmp(argv[0], "replaceView")) {
      ...
      if (!view1 || !view2) {
	control->ReturnVal(API_ERROR, "TkControl:Cannot find view for replaceView");
	return API_ERROR;
      }
      ViewWin *win = view1->GetParent();
      if (!win) {
	control->ReturnVal(API_ERROR, "TkControl:Cannot find window for replaceView");
	return API_ERROR;
      }
	...
	control->ReturnVal(API_OK,label->GetName());
      return API_OK;
    }
	...
	return API_ERROR;
      ...
      if (!action) {
	control->ReturnVal(API_ERROR, "Cannot find action");
	return API_ERROR;
      }
    ...
    if (!strcmp(argv[0], "highlightView")) {
      View *view = (View *)classDir->FindInstance(argv[1]);
      if (!view) {
	control->ReturnVal(API_ERROR, "Cannot highlight view");
	return API_ERROR;
      }
	...
	return API_ERROR;
      ...
      control->ReturnVal(API_OK,view->GetMappingLegend(map));
      return API_OK;
    }
  ...
  if (argc == 4) {
	...
	return API_ERROR;
      ...
      if (!label) {
	control->ReturnVal(API_ERROR, "Cannot find label");
	return API_ERROR;
      }
      if (!strcmp(argv[3], "x"))
    ...
    if (!strcmp(argv[0], "swapView")) {
      if (argc != 4) {
	control->ReturnVal(API_ERROR, "Invalid number of parameters");
	return API_ERROR;
      }
	...
	control->ReturnVal(API_ERROR, "Cannot find view or window");
	return API_ERROR;
      }
      if (view1->GetParent() != viewWin || view2->GetParent() != viewWin) {
	control->ReturnVal(API_ERROR, "Views not in same window");
	return API_ERROR;
      }
  ...
  if (argc == 6) {
	...
	control->ReturnVal(API_ERROR, "invalid date or float");
	return API_ERROR;
      }
