...
Make output to file
****************************************************************/
void MakeOutput(int argc, char **argv, int bufSize, double gdataLow,
...
while (curWinSize  <= maxWinSize){
		...
		if (waitTime > 0)
			fprintf(file,"wait %d\n",waitTime);
		fprintf(file,"s1 scrollx .5 scrollx %.2f\n",scroll);
		if (waitTime > 0)
			fprintf(file,"wait %d\n",waitTime);
		fprintf(file,"s1 scrollx .5 scrollx %.2f\n",scroll);
		if (waitTime > 0)
...
main(int argc, char **argv){
	...
	sprintf(cmdFile,"%s.cmd.np",argv[6]);
	FILE *file = fopen(cmdFile,"w");
	if (file == NULL){
		fprintf(stderr,"can't open file %s\n",cmdFile);
		exit(1);
	}
	MakeOutput(argc,argv,bufSize, gdataLow,file,minWinSize, maxWinSize, 0);
	fclose(file);

	sprintf(cmdFile,"%s.cmd.yp",argv[6]);
	file = fopen(cmdFile,"w");
	if (file == NULL){
		fprintf(stderr,"can't open file %s\n",cmdFile);
		exit(1);
	}
	MakeOutput(argc,argv,bufSize, gdataLow,file,minWinSize, maxWinSize, waitTime);
	fclose(file);
}
