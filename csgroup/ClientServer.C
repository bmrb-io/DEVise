...
Server::Server(char *name, int port, char* switchname, int maxClients)
{
    _name = strdup(name);
    ...
    // support switch board functions
    groupname = NULL;
    passivefd = -1;
    RPCInit(switchname);
    rpcenabled = 1;
...
int Server::RegisterServer(char* grpname, int mode)
{
    ConnectInfo address;
    char hostname[MAXNAMELEN];
    struct hostent *hst;

    // save a copy of registration information
    if (!rpcenabled)
      return -1;
    ...
    if (groupname !=NULL)
    {
      fprintf(stderr, "Server already registered group:%s\n",groupname);
    }
    else
    {
      this->groupname = new char(strlen(grpname)+1);
      strcpy(this->groupname, grpname);
      this->address = address;
      this->mode = mode;
      passivefd = Register(groupname, address, mode);
#ifdef DEBUG
if (mode ==0)
  printf("passive register return passivefd=%d\n",passivefd);
#endif
   }
   return passivefd;
}

int Server::DeRegisterServer()
{
    int result=-1;
    ...
    if (groupname != NULL)
    {
      result = DeRegister(groupname, address, mode);
      delete groupname;
      groupname = NULL;
    }
    return result;
...
void Server::MainLoop()
{
    ...
    while (1) {
	// wait for connection iff we are an active server
	if ((!rpcenabled)||(passivefd == -1)) 
          WaitForConnection();

	// if more capacity for clients or in passive mode
        while ((_numClients > 0)||
               ((passivefd !=-1)&&(rpcenabled)))
        {
	    SingleStep();
        }
    }
...
// for passive server, no need to close fd
...
// for active or master server, close file descriptor
...
void Server::ReadCmd()
{
    ...
    // support switch board functions
    if ((passivefd !=-1)&&(rpcenabled)) 
    {
	FD_SET(passivefd, &fdset);
	if ( passivefd > maxFdCheck)
	{
	     maxFdCheck = passivefd;
	}
    }
    ...
    // filtering switch board messages
    if ((rpcenabled)&&((passivefd !=-1)&&(FD_ISSET(passivefd, &fdset))))
    {
	// for passive server
	result = NetworkReceive(passivefd, 1, flag, argc, argv);
	if (result<0)
	{
	  printf("passive server receive error\n");
	}
...
#endif
            // propagating messages to the switch board, if registered
	    if ((groupname != NULL)&&(rpcenabled))
	    {
	      char * recBuffer;
	      int msgsize;
	      msgsize = NetworkPrepareMsg(API_CMD, 0,argc,argv, &recBuffer);
	      if (GroupSend(groupname,recBuffer,msgsize)<0)
	      {
		printf("Group send error\n");
	      }
#ifdef DEBUG
              else
	      {
...
//-----------------------------------------------------------------------
WinServer::WinServer(char *name, int port, char *switchname, int maxClients)
: Server(name, port, switchname, maxClients)
#endif
{
