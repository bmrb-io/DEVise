...
/*
  ...
  $Log$
  Revision 1.6  1997/08/12 15:32:37  wenger
...
class Server {
  ...
  private:
    ...
    ConnectInfo address;
    int    mode;
    int    passivefd;    // -1, if active mode, otherwise  fd in passive mode
    int    rpcenabled;
  public:

    Server(char *name, int port, char *switchname, int maxClients = 10);
    virtual ~Server();
    virtual void MainLoop();              // main loop of server
    virtual void SingleStep();            // once throught the body of MainLoop
    virtual int  RegisterServer(char* groupname, int mode);
    virtual int  DeRegisterServer();
  ...
  protected:
    ...
    virtual void ProcessCmd(int argc,
			    char **argv);  // process passive client command
...
class WinServer : public Server, public WindowRepCallback {
  ...
  public:
    ...
    WinServer(char *name, int port, char *switchname,  int maxClients = 10);
    virtual ~WinServer();
    virtual void SingleStep();            // once throught the body of MainLoop
