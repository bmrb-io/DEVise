...
class SampleWinServer : public WinServer {
  ...
  public:  
    ...
    SampleWinServer(char *name, int port,char* switchname, int maxClients = 10)
	: WinServer(name, port,switchname, maxClients)
	{}
...
void Usage()
{
    fprintf(stderr, 
    "Usage: %s -p port"
    " -s switch's hostname [-g groupname] -a|-i \n"
    "\t-a: active connect\n\t-i: passive connect\n", _progName);
}
...
int main(int argc, char **argv)
{
	...
	else if (!strcmp(argv[i], "-s"))
	{
	    if (i + 1 >= argc) {
	      errors = 1;
	      fprintf(stderr, "Switch hostname expected\n");
	      break;
	    }
	    strcpy(switchname,argv[i + 1]);
	    i++;
	}
	else if (!strcmp(argv[i], "-g"))
	{
	    if (i + 1 >= argc) {
	      errors = 1;
	      fprintf(stderr, "Group name expected\n");
	      break;
	    }
	...
	else if (!strcmp(argv[i], "-a"))
	{
	    ...
	    {
	      errors = 1;
	      fprintf(stderr, "Contradictory mode setting\n");
              break;
	    }
	    mode = 1;
	}
    ...
    if ((errors)||(switchname[0] == 0))
    {
      Usage();
      exit(1);
    }
    
    Server server("Server", port, switchname,10);
    if (server.RegisterServer((char*)groupname, mode)<0)
    {
      fprintf(stderr, 
      "Registration failure, mode=%d, switch=%s, group=%s\n",
	 mode, switchname, groupname);    
      exit(-1);
    }
    server.MainLoop();
    return 1;
