...
/*
  ...
  $Log$
  Revision 1.19.4.1  2014/01/17 21:45:59  wenger
  Fixed a bunch of possible buffer overflows.

  Revision 1.19  2008/10/13 19:45:34  wenger
  More const-ifying, especially Control- and csgroup-related.

  Revision 1.18  2008/09/11 19:56:56  wenger
...
static const char* errBase[Client::MAX_ERRORS]=
{
...
Client::NotifyGrabbed()
{
	...
	if (_groupname != NULL)
	{
		int formatted = snprintf(info, sizeof(info),
		  "Leader for group:%s ",_groupname);
		assert(formatted < (int)sizeof(info));//TEMP
		//TEMP checkAndTermBuf2(info, formatted);
...
Client::RequestRelinquish()
{
	...
	{
		int formatted = snprintf(info, sizeof(info),
		  "Leadership for group:%s is requested!", _groupname);
		assert(formatted < (int)sizeof(info));//TEMP
		//TEMP checkAndTermBuf2(info, formatted);
		SetPanelSubCmd("normal");
		SetPanelMajorCmd("StatusLine");
...
void Client::DoAbort(const char *reason)
{
  fprintf(stderr, "%s\n", reason);
  (void)NetworkClose(_serverFd);
  exit(1);
...
Client::ServerCmd(int argc, const char * const *argv)
{
	...
	if (!strcmp(argv[0],"GroupCmd"))
	{
		...
		else if (!strcmp(argv[1],"CreateGroup"))
		{
			...
			int formatted = snprintf(info, sizeof(info),
			  "Group:%s created successfully", argv[2]);
		    assert(formatted < (int)sizeof(info));//TEMP
			//TEMP checkAndTermBuf2(info, formatted);
			if (retval <0)
			{
		...
		else if (!strcmp(argv[1],"LeadGroup"))
		{
			...
			if (_groupname != NULL)
			{
				...
				int formatted = snprintf(info, sizeof(info),
				  "Requested leadership for group:%s", _groupname);
		        assert(formatted < (int)sizeof(info));//TEMP
				//TEMP checkAndTermBuf2(info, formatted);
				if (retval <0)
				{
		...
		else if (!strcmp(argv[1],"ListenGroup"))
		{
			...
			if (_groupname != NULL)
			{
				...
				int formatted = snprintf(info, sizeof(info),
				  "Gave up leadership for group:%s", _groupname);
		        assert(formatted < (int)sizeof(info));//TEMP
				//TEMP checkAndTermBuf2(info, formatted);
				if (retval <0)
				{
