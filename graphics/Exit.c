...
/*
  ...
  $Log$
  Revision 1.32  2008/09/11 19:57:06  wenger
...
{
  char fulltext[256];
  int formatted = snprintf(fulltext, sizeof(fulltext), "%s (%s:%d)",
      reason, file, line);
  checkAndTermBuf2(fulltext, formatted);
...
#if !defined(LIBCS) && !defined(ATTRPROJ)
  char errBuf[1024];
  formatted = snprintf(errBuf, sizeof(errBuf), "Failed assertion: %s\n",
      fulltext);
