...
/*
printf("after view register 0x%x\n", this);
*/
}
...
inline FindRealCoord(WindowRep *winRep, Coord xlow, Coord ylow, 
	Coord xhigh, Coord yhigh, int &txlow, int &tylow, int &txhigh, int &tyhigh){
	Coord xl,yl, xh, yh;
...
*/
	...
	*/
	...
	if (Geom::RectRectIntersect(drawX, drawY, drawX+drawW-1,drawY+drawH-1,
			yCursorX,yCursorY,yCursorX+yCursorW-1,yCursorY+yCursorH-1))
		doYCursor = true;
...
View::PixmapStat View::RestorePixmap(VisualFilter filter, VisualFilter &newFilter){
	int x, y;
