...
XWindowRep:: XWindowRep(Display *display,Window window, XDisplay * DVDisp,
		Color fgndColor, Color bgndColor, Boolean backingStore):
...
#ifdef DEBUG_XWINDOW
	printf("XWindowRep::PUshClip(%f,%f,%f,%f)\n",x,y,w,h);
#endif
...
#ifdef DEBUG_XWINDOW
	printf("Xwindow::PushClip: transformed into (%f,%f,%f,%f)\n",
			xlow,ylow,width,height);
#endif

	XRectangle rect ;
	rect.x = (short)xlow; rect.y= (short)ylow;
	...
	*/
	...
	if (_dispGraphics)
		...
		for (i=0; i < num; i++){
			Coord tx,ty;
			WindowRep::Transform(x[i],y[i],tx,ty);
			rectAngles[i].x = ((short)tx)-halfWidth;
			rectAngles[i].y = ((short)ty)-halfWidth;
			rectAngles[i].width = width;
			rectAngles[i].height = width;

			/*
			printf("xform to (%d,%d,%d,%d)\n", rectAngles[i].x,
...
virtual void XWindowRep::FillRectArray(Coord *xlow, Coord *ylow, Coord *width, 
	...
	for (i=0; i < num; i++){
		...
		rectAngles[index].x = (short)txlow;
		rectAngles[index].y = (short)tylow;
		rectAngles[index].width = (unsigned short)pixelWidth;
		rectAngles[index].height = (unsigned short)pixelHeight;

/*
printf("FillRect %f,%f,%f,%f --> %d,%d,%d,%d\n",
		xlow[i], ylow[i],width[i],height[i],
		rectAngles[index].x, rectAngles[index].y, rectAngles[index].width,
...
virtual void XWindowRep::FillRectArray(Coord *xlow, Coord *ylow, Coord width,
	...
	for (i=0; i < num; i++){
		...
		rectAngles[i].x = (short)txlow;
		rectAngles[i].y = (short)tylow;
		rectAngles[i].width = (unsigned short)pixelWidth;
		rectAngles[i].height = (unsigned short)pixelHeight;
	}
#ifdef GRAPHICS
	if (_dispGraphics)
		XFillRectangles(_display,_win, _gc, rectAngles,num);
...
#ifdef DEBUG_XWINDOW
	printf("XWindowRep::fillREct %f,%f,%f,%f\n",xlow,ylow,width,height);
#endif
    Coord txlow, tylow, txmax, tymax;
	Coord x1,y1,x2,y2;
...
#ifdef DEBUG_XWINDOWREP
printf("Xwindow::Fillpoly: %d points\n",n);
for (int j =0; j < n; j++){
...
printf("transformed into: ");
	...
	for (int i=0;  i < n; i++ ){
		...
		_xpoints[i].x = (short) tx;
		_xpoints[i].y = (short) ty;
#ifdef DEBUG_XWINDOWREP
...
#ifdef DEBUG_XWINDOWREP
printf("Xwindow::FillPixelPoly: %d points\n",n);
for (int j =0; j < n; j++){
	printf("(%f,%f) ",points[j].x,points[j].y);
};
printf("\n");
...
#endif
	for (int i=0;  i < n; i++ ){
		_xpoints[i].x = (short) points[i].x;
		_xpoints[i].y = (short) points[i].y;
#ifdef DEBUG_XWINDOWREP
printf("(%d,%d) ",_xpoints[i].x, _xpoints[i].y);
#endif
	}
...
/*
printf("scale= %f, textWidth = %d, textHeight = %d, dstw=%d, dstH = %d\n",
...
printf("XwindowRep::Scroll(%f,%f,%f,%f,%f,%f)\n", x,y,w,h,dstX,dstY);
    Coord xlow,ylow,xhi,yhi,width,height;
	Coord x1, y1, x2,y2;
	WindowRep::Transform(x,y, x1, y1);
	WindowRep::Transform(x+w,y+h, x2,y2);
