...
XDisplay::XDisplay(char *name){
	if ((this->_display = XOpenDisplay(name))==NULL){
	...
	/* get font */
	if ((_fontStruct = XLoadQueryFont(_display,"8x16"))==NULL){
		fprintf(stderr,"XDisplay: can not load font\n");
		Exit::DoExit(1);
	}

	/* init stipples for patterns*/
	Window win; /* temp window for creating stipples */
#if defined(HPUX) || defined(IBM_POWER_STATION) || defined(ALPHA) || defined(PENTIUM)
	if ((win =XCreateSimpleWindow(_display, DefaultRootWindow(_display), 
		(unsigned)0, (unsigned)0, (unsigned)10, (unsigned)10, 
		/* border width */ 5, /* border */ 0, /* background */ 0))==0 )
#else
	if ((win =XCreateSimpleWindow(_display, DefaultRootWindow(_display), 
		(unsigned)0, (unsigned)0, (unsigned)10, (unsigned)10, 
		/* border width */ 5, /* border */ 0, /* background */ 0))==NULL)
#endif
		{
	...
	for (int i =0; i < XNumBitmaps; i++){
		_stipples[i] = XCreateBitmapFromData(_display,win,
		xBitmaps[i], XBitmapWidth,XBitmapHeight);
	}
...
Colormap cmap= DefaultColormap(this->_display,DefaultScreen(this->_display));
XColor color_def;
Color color;
...
Create a new window 
***************************************************************/
WindowRep *XDisplay::CreateWindowRep(char *name,
	Coord x, Coord y, Coord width, Coord height, 
	Color fgnd, Color bgnd, 
	WindowRep *parentRep, Coord min_width, Coord min_height,
...
XWindowRep *xParentRep= (XWindowRep *)parentRep;
	...
	*/
	...
	if (winBoundary){
#if defined(IBM_POWER_STATION) || defined(ALPHA) || defined(HPUX) || defined(PENTIUM)
		border_width = (parent == 0? 5: 1);
#else
		border_width = (parent == NULL? 5: 1);
#endif
	}
	else {
#if defined(IBM_POWER_STATION) || defined(ALPHA) || defined(HPUX)||defined(PENTIUM)
		border_width = (parent == 0? 5: 0);
#else
		border_width = (parent == NULL? 5: 0);
#endif
	}

	if ((w = XCreateWindow(_display, parent, (unsigned)realX, (unsigned)realY, 
		(unsigned)realWidth, (unsigned)realHeight, /* border width */ 
		border_width,
		0, InputOutput, CopyFromParent, AllPlanes , &attr)) == 0){
	...
	*/
	...
	if (XMapWindow(_display, w) < 0) return NULL;
...
/**************************************************************
 Destroy a window. Parameter "win" better be of type "class Xwindow".
**************************************************************/
void XDisplay::DestroyWindowRep(WindowRep *win) {
	class XWindowRep *xwin = (XWindowRep *)win;
	...
	while (XPending(_display)> 0){
		...
		for (index=_winList.InitIterator(); _winList.More(index);){
			...
			if (win->_win == event.xany.window){
				...
				because it might trigger a call to DestoryWinowRep() to
