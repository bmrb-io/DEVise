...
static char buf2[80], buf3[80], buf4[80], buf5[80];
...
ClassInfo *WinVerInfo::CreateWithParams(int argc, char **argv){
	...
	ViewWinVer *win = new ViewWinVer(name,
...
#ifdef WIN_HOR
InstanceInfo *WinHorInfo::CreateWithParams(int argc, char **argv){
	if (argc != 5){
		fprintf(stderr,"WinHorInfo::CreateWithParams wrong args\n");
		return NULL;
	}
	char *name = CopyString(argv[0]);
	ViewWinHor *win = new ViewWinHor(name,
		atof(argv[1]),atof(argv[2]),atof(argv[3]),atof(argv[4]));
	return new InstanceInfo(name, win);
}
#endif
