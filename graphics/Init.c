...
/*
  ...
  Revision 1.74.8.1  2012/04/23 21:22:54  wenger
  ...
  Revision 1.74  2010/02/23 22:05:32  wenger
...
const char *Init::_jsDataDownloadDir = ".";
const char *Init::_jsDataDownloadUrl = "file:/";
...
void Init::DoInit(int &argc, char **argv)
{
  ...
  // Get name of JavaScreen data download directory.
  const char *jsDataDownloadDir = getenv("DEVISE_JS_DATA_DOWNLOAD_DIR");
  if (jsDataDownloadDir) {
    _jsDataDownloadDir = CopyString(jsDataDownloadDir);
  }

  // Get JavaScreen data download URL.
  const char *jsDataDownloadUrl = getenv("DEVISE_JS_DATA_DOWNLOAD_URL");
  if (jsDataDownloadUrl) {
    _jsDataDownloadUrl = CopyString(jsDataDownloadUrl);
  }
