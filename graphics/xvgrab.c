...
int  gbits;                              /* either '8' or '24' */
byte grabmapR[256], grabmapG[256], grabmapB[256];  /* colormap */
int  gWIDE,gHIGH;
#else
...
/**************************************/
#ifdef MODIFIED
int convertImage(image, colors, ncolors, xwap)
#else
     ...
     XWindowAttributes *xwap;
{
  ...
  int            r8shift = 0, g8shift = 0, b8shift = 0;
#else
...
#ifdef MODIFIED
int getxcolors(win_info, colors, theDisp)
...
#ifdef DEBUG
    fprintf(stderr,"no colormap associated with window\n");
#endif
    return 0;
  }
...
#ifdef DEBUG
  fprintf(stderr,"%d entries in colormap\n", ncolors);
#endif
