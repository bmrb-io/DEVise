...
#ifdef MODIFIED
#define FatalError(msg)     { fprintf(stderr, "%s", msg); exit(1); }
#else
...
#ifdef MODIFIED
#define SetISTR(code,msg) puts(msg)
#else
#if defined(__STDC__) && !defined(NOSTDHDRS)
void  SetISTR(int, ...);
#else
...
#ifdef MODIFIED
#define ErrPopUp(msg1,msg2) fprintf(stderr, "%s%s", msg1, msg2)
#else
...
extern byte *grabPic;
extern int  gbits;
extern byte grabmapR[256], grabmapG[256], grabmapB[256];
extern int  gWIDE,gHIGH;

#ifdef MODIFIED
extern int    convertImage    PARM((XImage *, XColor *, int, 
				    XWindowAttributes *));
extern int    getxcolors      PARM((XWindowAttributes *, XColor **,
				    Display *));
#endif
