...
#include "docindex.h"
#include "inter.h"
#include "network.h"    /* sanjay */
#include <sys/msg.h>    /* sanjay */

int smartsock,newsock,smartportnum;

static long verbose;         /* Indicate level of description to user */

struct smart_msg_buf  /* sanjay added this type */
{
 long mtype;
 int  recv_portnum;
};
...
int inst;
{
  ...
  /* sanjay added these here */
    ...
    */
    printf("Finding a socket for connection \n");

    GetServerTCPPort(&smartportnum,&smartsock);

    printf("Found port   number =%d sending it to the parent \n",smartportnum);
    printf("Found socket number =%d \n",smartsock);
      
    pmb.mtype = 1;
    pmb.recv_portnum = smartportnum;
    ...
    while (1) {
        ...
        */
        ...
        /* Sanjay - trying some change here :
           try to read from the network raw socket..
           PATH_LEN is 1024   */
        /*
        ...
        */
        /* sanjay .. interrupting here */

        /* 
           find a socket from starting from an initial sock number
           such that the socket is not in use and return it to the parent
           mini-server
         */

        printf("Waiting for some command \n");
        fflush(stdout);
        ...
        ReadFromTCPSocket(newsock,in_buffer,PATH_LEN);
        printf("command = %s\n",in_buffer);
        fflush(stdout);
        sprintf(mac_name,"1");
        WriteToTCPSocket(newsock,mac_name,12);
        ...
        */
        ...
        if (command->save_output == 0 || status == 0) 
            /* 
             (void) show_output (&is.err_buf);
             */
            (void) send_output (&is.output_buf,mac_name,newsock);
        else
           /* Sanjay :changing some stuff here
...
static int
send_output (buf,mac_name,sock)
SM_BUF *buf;
char   *mac_name;
int    sock;
{
    int num_written, newly_written;
    char *tempBuffer, numbytestosend[12];
    ...
    */
        ...
        sprintf(numbytestosend,"%d",buf->end);
        WriteToTCPSocket(newsock,numbytestosend,12);
        tempBuffer = (char *)malloc(buf->end);
        bzero(tempBuffer,buf->end);
        printf("num bytes = %d\n",buf->end);
        memcpy(tempBuffer,buf->buf,buf->end);
        WriteToTCPSocket(newsock,tempBuffer,buf->end);
        free(tempBuffer);
        /* CloseSocket(sock); */
        /* CloseSocket(newsock); */
        printf("bytes = %s\n",buf->buf);
