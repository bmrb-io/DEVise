...
void GetServerTCPPort(int *smartportnum,int *smartsock)
{
 int init_portnum = 11000;
 int current_portnum;
 ...
 struct sockaddr_in my_addr;
  ...
  current_portnum = init_portnum;

  while(1)
  {
   my_addr.sin_port       =htons(current_portnum);
...
int ReadFromTCPSocket(int sock,char *msg, int bufferlength)
{
  ...
  if (nbytes < 0) printf("network error: recv failed on socket");
  return nbytes;
}
...
int WriteToTCPSocket(int sock,char *msg, int bufferlength)
{
  ...
  nbytes = send( sock, msg, bufferlength, 0);
  if (nbytes < 0) printf("network error: send failed on socket");
  return nbytes;
}
