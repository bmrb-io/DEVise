 ...
 *1 index.parse_sect.adj
 *2 adj_parse (token, consect, inst)
 *3   SM_TOKENSECT *token;
 *3   SM_CONSECT *consect;
 *3   int inst;
 *4 init_adj_parse(spec, unused)
...
int 
init_adj_parse(spec, param_prefix)
SPEC *spec;
char *param_prefix;
{
    STATIC_INFO *ip;
    ...
    PRINT_TRACE (2, print_string, "Trace: entering init_adj_parse");
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving init_adj_parse");

    return (new_inst);
}

/* WARNING: for efficiency, adj_parse changes the input token IN PLACE */

int
adj_parse (tokensect, consect, inst)
...
int inst;
{
    ...
    PRINT_TRACE (2, print_string, "Trace: entering adj_parse");
    PRINT_TRACE (6, print_tokensect, tokensect);

    if (! VALID_INST (inst)) {
        set_error (SM_ILLPA_ERR, "Instantiation", "adj_parse");
        return (UNDEF);
    }
    ip  = &info[inst];

    /* Reserve space for conlocs.  Note, in adj_parse (as opposed to 
    ...
    PRINT_TRACE (2, print_string, "Trace: leaving adj_parse");

    return (1);
}

int 
close_adj_parse(inst)
int inst;
{
    STATIC_INFO *ip;
    long i;
                                ...
                                add phrases */
{
    ...
    if (ip->sent_num != ip->phrase_sentence) {
        ...
        ip->num_phrase_comp = 0;
        if (UNDEF == add_comp (ip, new_stem, token_num))
            return (UNDEF);
        return (1);
    }

    token_range = token_num - 2 * phrase_distance;
    ...
    for (i = 0; i < ip->num_phrase_comp; i++) {
        ...
        if (ip->comp_loc[i] >= token_range) {
            ...
            PRINT_TRACE (20, print_long, &i);
            if (0 < strcmp (ip->phrase_comp[i], new_stem))
                (void) sprintf (phrase_buf, "%.100s %.100s",
                                new_stem, ip->phrase_comp[i]);
            else
                (void) sprintf (phrase_buf, "%.100s %.100s",
                                ip->phrase_comp[i], new_stem);
            if (UNDEF == add_con (ip,
                                  phrase_buf,
                                  &ip->parse_func[SM_PT_F_PHRASE],
                                  token_num,
