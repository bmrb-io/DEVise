...
typedef struct {
    ...
    char *filter_file;           /* Filename in which output of filter
                                    is to be placed */
    int lookup_table[128];       /* lookup_table[c] is index pointing to
...
char *pp_infile;
{
    ...
    if (pp_infop->filter != NULL && pp_infop->filter[0] != '\0') {
        ...
        if (NULL == (ip->filter_file = malloc (PATH_LEN)))
            return ( UNDEF);
        (void) sprintf (ip->filter_file, "/tmp/smfi.%d.%d", getpid(),new_inst);
    }
    else
        ip->filter = NULL;
...
STATIC_INFO *ip;
{
    ...
    int status;
    if (isascii (*text) &&
        (ip->pp_info->num_pp_sections > (i = ip->lookup_table[*text]))) {
...
STATIC_INFO *ip;
{
    ...
    for (i = 0; i < num_sections; i++) {
        ...
        if (ip->lookup_table[c] == num_sections) {
            ip->lookup_table[c] = i;
        }
    }
    return (0);
