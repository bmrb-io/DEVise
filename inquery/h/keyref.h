...
/* to be used to provide access to the keyfile package */
...
static Char_t keyref_h_rcsid[]="$RCSfile$, $Revision$, $Date$";

#include "docdef.h"
#include "keyerr.h"

/* run the program keyfile-sizes to determine the right values 
 * for these if there are changes to the fcb or block defns 
 */
#ifdef INQ_ARCH_alpha
  #define min_fcb_lc 7444
#else
  #define min_fcb_lc 7008
#endif
#define block_lc 2044

#define  FCBLEN (min_fcb_lc + 5*block_lc)

#define max_key_lc 82                /* maximum key length */
enum file_access {ran,seq};
enum file_open_mode {ro,rw};
struct pointer {
  Int32_t sc;
  Int32_t lc;
  Int32_t file_id;  /* MULTI_FILE */
};
