...
#ifndef INQ_HIGHLIGHT_H
#define INQ_HIGHLIGHT_H

/* This defines Char_t and so forth */

#include "doctypes.h"

/* This defines matchinfo_t */

#include "inq_api.h"

/* Function prototypes to inq_highlight.c file. */

Int_t
get_pass_matches (Int_t n_givens, matchinfo_t * givens,
                Int_t want_results, matchinfo_t * results);

Int_t
find_phrase (Char_t *text, Int_t starting,
                Char_t *phrase, Int_t *length);

Int_t
get_tok_matches (int n_given, matchinfo_t * given,
                int want_results, matchinfo_t * results);

void
copy_match (matchinfo_t to, matchinfo_t from);
