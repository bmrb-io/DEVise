...
/*
  ...
  $Log$
  Revision 1.2  1995/09/05 20:31:45  jussi
  Added CVS header.
*/
...
TDataBinaryInterpClassInfo::~TDataBinaryInterpClassInfo(){
/*
printf("TDataBinaryInterpClassInfo: destructor\n");
*/
	if (_tdata != NULL)
		delete _tdata;
}

char *TDataBinaryInterpClassInfo::ClassName() { return _className; }
...
ClassInfo *TDataBinaryInterpClassInfo::CreateWithParams(int argc, char **argv){
    if (argc != 2){
		fprintf(stderr,"TDataBinaryInterp::CreateWithParams: wrong args\n");
...
TDataBinaryInterp::TDataBinaryInterp(char *name, int recSize, AttrList *attrs,
	...
	for (i=0; i < _numAttrs; i++){
		...
		if (info->hasMatchVal){
			if (_numMatchingAttrs >= MAX_MATCHING_ATTRS){
				fprintf(stderr,"TDataBinaryInterp: too many matching attrs\n");
				Exit::DoExit(2);
			}
...
/*
printf("TDataBinaryInterp: destructor\n");
*/
}
		...
		for (i=0; i < _numMatchingAttrs; i++){
			...
			switch(info->type){
				...
				case StringAttr:
					...
					break;
				case DateAttr:
					fprintf(stderr,"TDataBinaryInterp: date attr not implemented\n");
					Exit::DoExit(2);
					break;
				default:
					fprintf(stderr,"TDataBinaryInterp: unknown attrType\n");
					Exit::DoExit(2);
					break;
			}
		}
			...
			switch(attrInfo->type){
				...
				case DateAttr:
					...
					*(time_t *)ptr = tm;
					break;
				default:
					fprintf(stderr,"TDataBinaryInterp: unknown attribute type\n");
					Exit::DoExit(2);
			}
		}
	}
