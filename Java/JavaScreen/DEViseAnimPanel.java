...
public class DEViseAnimPanel extends Canvas implements Runnable
{
    ...
    public DEViseAnimPanel(jsdevisec what, Vector array, int time)
    {
        ...
        if (imageCount == 0) {
            isAnimated = false; 
            YDebug.println("Empty image list received in DEViseAnimPanel constructor!");
            return;
        }
        ...
        if (currentImg == null) {
            isAnimated = false; 
            YDebug.println("NULL image received in DEViseAnimPanel constructor!");
            return;
        }
        ...
        if (imageWidth <= 0 || imageHeight <= 0) {
            isAnimated = false;
            YDebug.println("Empty image received in DEViseAnimPanel constructor!");
            return;
        }
        ...
        for (int i = 1; i < imageCount; i++)  {
            ...
            if (width != imageWidth || height != imageHeight) {
                isAnimated = false;
                YDebug.println("Different image size encounter in DEViseAnimPanel constructor!");
                return;
            }
        }
    ...
    // start() and stop() will only be accessed by DEViseCmdDispatcher
    public void start()
    {   
        if (!isAnimated)
            return;
