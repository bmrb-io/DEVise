...
public class DEViseCDataChannel implements Runnable
{
    ...
    DataChannelStopper stopper = null;
    ...
    {        
        ...
        if (Thread.currentThread() == dataChannel) {
            if (stopper != null && stopper.isAlive()) {
                stopper.stop();
                stopper = null;
            }
            
            stopper = new DataChannelStopper(this);
            stopper.start();
        } else {
            if (dataChannel.isAlive()) {
                dataChannel.stop();
            }
...
class DataChannelStopper extends Thread
{
    DEViseCDataChannel channel = null;
    
    public DataChannelStopper(DEViseCDataChannel what)
    {
        channel = what;
    }
    ...
    public void run()
    {
        if (channel.dataChannel != null && channel.dataChannel.isAlive()) {
            channel.dataChannel.stop();
        }
