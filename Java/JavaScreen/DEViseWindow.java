...
public class DEViseWindow extends Container
{
    ...
    boolean isWithinCursor = false, isMovingCursor = false, isMsgShow = false;
    ...
    private int checkCursor(Point p)
    {
        ...
        int edge = 4;
        for (int i = 0; i < viewCursor.length; i++) {
            if (!viewCursor[i].isXMovable && !viewCursor[i].isYMovable)
                continue;
            ...
            if (p.x >= viewCursor[i].x && p.x <= (viewCursor[i].x + viewCursor[i].width)
                ...
                if (p.x >= viewCursor[i].x && p.x <= viewCursor[i].x + edge) {
                    if (p.y >= viewCursor[i].y && p.y <= viewCursor[i].y + edge) {
                        whichSide = 5;
                        return 2;
                    } else if (p.y <= viewCursor[i].y + viewCursor[i].height && p.y >= viewCursor[i].y + viewCursor[i].height - edge) {
                        whichSide = 6;
                        return 2;
                    } else {
                        if (viewCursor[i].isXMovable) {
                            whichSide = 1;
                            return 2;
                        } else {
                            return 1;
                ...
                } else if (p.x <= viewCursor[i].x + viewCursor[i].width && p.x >= viewCursor[i].x + viewCursor[i].width - edge) {
                    if (p.y >= viewCursor[i].y && p.y <= viewCursor[i].y + edge) {
                        whichSide = 7;
                        return 2;
                    } else if (p.y <= viewCursor[i].y + viewCursor[i].height && p.y >= viewCursor[i].y + viewCursor[i].height - edge) {
    ...
    private void updateCursorPosition(Point p)
    {
        ...
        if (isWithinCursor) {
            if (viewCursor[whichCursor].isXMovable) {
            ...
            if (viewCursor[whichCursor].isYMovable) {
            ...
            if (isMouseDragged) {
                if (currentView != null) {
                    ...
                    if (isWithinCursor || isMovingCursor) {
                        ...
                        //            + "\nJAVAC_CursorChanged {" + currentView.getName() + "} " + viewCursor[whichCursor].x
                        //            + " " + viewCursor[whichCursor].y + " " + viewCursor[whichCursor].width + " " + viewCursor[whichCursor].height);
                        //} else {
                            dispatcher.insertCmd("JAVAC_CursorChanged {" + currentView.getName() + "} " + viewCursor[whichCursor].x
                                    + " " + viewCursor[whichCursor].y + " " + viewCursor[whichCursor].width + " " + viewCursor[whichCursor].height);
                        //}
                    } else {
                        repaint();
