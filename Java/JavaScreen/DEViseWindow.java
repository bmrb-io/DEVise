...
{   
    ...
    boolean isCurrent = false, isPopup = false, isMouseDragged = false;
    int userAction = 0;
    ...
    {
        if (what == null || name == null || loc == null || img == null || views == null)
            YDebugInfo.println("NULL argument received in DEViseWindow construction!");
            
        jsc = what;
        jscreen = what.jscreen;
        dispatcher = what.dispatcher;
        windowName = name;
        windowLoc = loc;
        winImage = img;        
        allViews = views;
    ...
    {
        if (name == null) {
            YDebugInfo.println("Null argument received in getView-DEViseWindow!");
            return null;
        } 
    ...
    {           
        if (img == null) {
            YDebugInfo.println("Null argument received in setImage-DEViseWindow!");
            return;
        } 
        ...
        if (imageWidth != windowDim.width || imageHeight != windowDim.height) {
            YDebugInfo.println("At this moment, the image size for a window can not be changed during a session!");
            return;
        }
