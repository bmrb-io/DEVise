...
void Image_BW::write_pic(char * file_name)
{
  ...
  FILE * out_file;
  if (!strcmp(file_name,"-")) out_file = stdout; 
  else out_file = fopen(file_name, "wb");
  ...
  if (strcmp(file_name,"-")) fclose(out_file);
...
void Image_BW::recover(void)
{
  ...
  int mx, my, nx = pdim.x >> levels, ny = pdim.y >> levels;
