...
/* Laplcian correction option for decompression of AC coeffs */
typedef struct {
  double delta[DCTSIZE2];             /* to be subtracted from the
					 dequantized coefficient */
  int least_val_to_correct[DCTSIZE2]; /* least quantized value to be
					 corrected. useful to ignore
					 the first bin as it might be
					 too narrow because of thresholding */
...
struct jpeg_decompress_struct {
  ...
  boolean do_fancy_pt_untrans;  /* TRUE=apply adjustment to pt untransform */

  #ifdef IJG_LAPL_CORRECT
  boolean do_lapl_correct;      /* TRUE=apply laplacian mean corrections to */
				/* reconstructed AC coefficients */
  lapl_correct_struct lapl_correct[3]; /* we only do laplacian correction for */
				       /* up to 3 components */
