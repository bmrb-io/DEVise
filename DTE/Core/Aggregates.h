...
public:
	  ExecMinMax(OperatorPtr opPtr, ADTCopyPtr copyPtr, 
	  ...
	  void initialize(const Type* input) {
		  // copy first value as current minMax
		  copyPtr(input, minMax, valueSize);	
	  }
	  
	  void update(const Type* input)  {
		  Type* boolVal;
		  opPtr(input, minMax, boolVal); 
		  if(boolVal) {
		    copyPtr(input, minMax, valueSize); 
		  }
	  }
...
public:
	  ExecSum(OperatorPtr addPtr, ADTCopyPtr copyPtr, 
		  DestroyPtr sumDestroy, Type* value, size_t valueSize) :
...
public:
	  ExecAverage(OperatorPtr addPtr,OperatorPtr divPtr, 
		      PromotePtr promotePtr, DestroyPtr sumDestroy, 
		      ADTCopyPtr copyPtr, Type* value, size_t valueSize) : 
...
public:
	  ExecMovSum(OperatorPtr addPtr, ADTCopyPtr copyPtr, 
		     DestroyPtr sumDestroy, Type* value, size_t valueSize, 
		     TupleLoader* tupLoad,OperatorPtr subPtr,OperatorPtr eqPtr) : 
		  ExecSum(addPtr, copyPtr, sumDestroy, value, valueSize), 
...
public:
	  ExecMovAverage(OperatorPtr addPtr,OperatorPtr divPtr,
			 PromotePtr promotePtr, DestroyPtr sumDestroy, 
			 ADTCopyPtr copyPtr, Type* value, size_t valueSize, 
			 TupleLoader* tupLoad,OperatorPtr subPtr,OperatorPtr eqPtr) : 
		  ExecAverage(addPtr, divPtr, promotePtr, sumDestroy, copyPtr,
			      value, valueSize), tupLoad(tupLoad), 
...
public:
	  ExecSeqAttr(ADTCopyPtr copyPtr, OperatorPtr eqPtr, Type* value, 
		  ...
		  ExecGroupAttr(copyPtr, eqPtr, value, valueSize), 
		  tupLoad(tupLoad) {}
...
public:
	  ...
	  ExecAggregate* createExec(){
		  ...
		  return new ExecAverage(addPtr, divPtr, promotePtr, sumDestroy, copyPtr, value, valueSize);
	  }
...
public:
	  ExecAggregate* createExec(){
		  ...
		  tupLoad = new TupleLoader;
		  TRY(tupLoad->open(numFlds, &typeID), NULL);
		  return new ExecMovSum(opPtr, copyPtr, sumDestroy, value, valueSize, tupLoad, subPtr, eqPtr);
	  }
...
public:
	  ExecAggregate* createExec(){
		  ...
		  return new ExecMovAverage(addPtr, divPtr, promotePtr,sumDestroy, copyPtr, value, valueSize, tupLoad, subPtr, eqPtr);
	  }
...
class CumeAggsExec : public StandAggsExec {
	  bool firstTime;
public: 	  
	  CumeAggsExec(Iterator* inputIter, ExecAggregate** aggExecs, 
		       int numFlds) :StandAggsExec(inputIter,aggExecs,numFlds){
		  firstTime = true;
	  }
...
protected:
	  Iterator* inputIter; // Assumes sorted on group attrs by optimizer
	  ExecAggregate** aggExecs;
	  int numFlds;
	  Tuple* retTuple;
...
public:
	  ...
	  StandGroupByExec(Iterator* inputIter, ExecAggregate** aggExecs,
		  ...
		  inputIter(inputIter), aggExecs(aggExecs),numFlds(numFlds),  
		  grpByPos(grpByPos),grpByPosLen(grpByPosLen), aggPos(aggPos), 
		  numAggs(numAggs) {
