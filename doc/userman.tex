...
the \menu{Swap Position} command in the \menu{View} menu. The two
...
\menu{View/Statistics} menu. This menu yields a {\em tear-off}
...
\scaleepspic[0.7\textwidth]{screen4.eps}{History Screen\label{screen4}}
...
by selecting \menu{Visualize/Import} or \menu{Visualize/Define}.
...
\code{ftp} or \code{http} protocol. The data is fetched from the Web
...
\menu{Schema} menu in the control panel. Every data stream is
...
The graphical data is the second intermediate stage in the processing
...
The view thus defined can then be modified using \menu{View/Edit
Mapping}. The mapping can be edited to change any one or all of the
GData attributes.
